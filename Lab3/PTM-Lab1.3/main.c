...
*/
	...
	************************** */
	...
	//Ustawienie PB5 jako wyjœcie stanu niskiego
	sbi(DDRB, 5);
	cbi(PORTB, 5);
	...
	while(bit_is_set(PIND,0)){		//Sprawdza czy przycisk zosta³ wcisniety
	...
	while (1) {
		...
		if(bit_is_clear(PIND,0)){		//Sprawdza czy przycisk zosta³ wcisniety

			sbi(PORTB, 5);			//Zmienia jako wyjœcie stan wysoki
			j++;
			_delay_ms(500);
		}
		if(j > 1){
			cbi(PORTB, 5);			//Zmienia jako wyjœcie stan niski
